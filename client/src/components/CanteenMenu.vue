<template>
    <div class="bg-black/20 text-xl font-bold p-4 border-b border-slate-700 pl-4 pr-2 py-2 rounded-t-lg grid header-with-badge">
    ...
    <div class="h-full overflow-y-auto flex flex-col">
        <div v-if="mealsDinner != null && mealsDinner.length > 0 && hours >= 17">
            <div v-for="(meal, index) in mealsDinner" :key="index" class="border-b border-slate-700 grid meal-item">
                <div class="px-4 py-2 pb-3 border-l-2" :class="(meal.isVegetarian && !meal.isVegan)?'border-lime-500':'' || (meal.isVegan)?'border-green-700':'' || (!meal.isVegetarian && !meal.isVegan)?'border-red-600':''">
                    <div class="grid meal-item">
                        <div class="pr-3 border-r border-slate-700">
        ...
        <div v-else-if="mealsLunch != null && mealsLunch.length > 0 && mergeProps.id != 53 || mealsLunch != null && mealsLunch.length > 0 && mergeProps.id == 53 && hours < 17">
            <div v-for="(meal, index) in mealsLunch" :key="index" class="border-b border-slate-700 grid meal-item">
                <div class="px-4 py-2 pb-3 border-l-4" :class="(meal.isVegetarian && !meal.isVegan)?'border-lime-500':'' || (meal.isVegan)?'border-green-700':'' || (!meal.isVegetarian && !meal.isVegan)?'border-slate-800':''">
                    <div class="grid meal-item">
                        <div class="pr-3 border-r border-slate-700">
